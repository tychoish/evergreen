...
func (uis *UIServer) spawnPage(w http.ResponseWriter, r *http.Request) {
	...
	if len(r.FormValue("distro_id")) > 0 {
		...
		if err != nil {
			uis.LoggedError(w, r, http.StatusInternalServerError,
				errors.Errorf("Error finding distro %v: %v", r.FormValue("distro_id"), err))
			return
		}
	}
	if len(r.FormValue("task_id")) > 0 {
		...
		if err != nil {
			uis.LoggedError(w, r, http.StatusInternalServerError,
				errors.Errorf("Error finding task %v: %v", r.FormValue("task_id"), err))
			return
		}
	}
...
func (uis *UIServer) getSpawnedHosts(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		uis.LoggedError(w, r, http.StatusInternalServerError,
			errors.Errorf("Error finding running hosts for user %v: %v", user.Username(), err))
		return
	}
...
func (uis *UIServer) listSpawnableDistros(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		uis.LoggedError(w, r, http.StatusInternalServerError, errors.Errorf("Error loading distros: %v", err))
		return
	}
...
func (uis *UIServer) requestNewHost(w http.ResponseWriter, r *http.Request) {
	...
	if putParams.SaveKey {
		...
		if err != nil {
			uis.LoggedError(w, r, http.StatusInternalServerError, errors.Errorf("Error fetching user: %v", err))
			return
		}
		err = model.AddUserPublicKey(dbuser.Id, putParams.KeyName, putParams.PublicKey)
		if err != nil {
			uis.LoggedError(w, r, http.StatusInternalServerError, errors.Errorf("Error saving public key: %v", err))
			return
		}
...
func (uis *UIServer) modifySpawnHost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		uis.LoggedError(w, r, http.StatusInternalServerError, errors.Errorf("error finding host with id %v: %v", hostId, err))
		return
	}
	if host == nil {
	...
	case HostPasswordUpdate:
		...
		if err != nil {
			uis.LoggedError(w, r, http.StatusInternalServerError, errors.Errorf("Error constructing host RDP password: %v", err))
			return
		}
		// update RDP and sshd password
		if err = pwdUpdateCmd.Run(); err != nil {
			uis.LoggedError(w, r, http.StatusInternalServerError, errors.Errorf("Error updating host RDP password: %v", err))
			return
		}
	...
	case HostExpirationExtension:
		...
		if err = host.SetExpirationTime(futureExpiration); err != nil {
			uis.LoggedError(w, r, http.StatusInternalServerError, errors.Errorf("Error extending host expiration time: %v", err))
			return
		}
