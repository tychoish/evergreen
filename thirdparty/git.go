...
func GitApplyNumstat(patch string) (*bytes.Buffer, error) {
	...
	if err != nil {
		return nil, errors.Errorf("Unable to create local patch file")
	}
	...
	for {
		...
		if err != nil && err != io.EOF {
			return nil, errors.Errorf("Unable to read supplied patch file")
		}
		if n == 0 {
			break
		}
	...
	if err := cmd.Start(); err != nil {
		return nil, errors.Errorf("Error validating patch: 424 - %v (%v)",
			summaryBuffer.String(), err)
	}

	// this should never happen if patch is initially validated
	if err := cmd.Wait(); err != nil {
		return nil, errors.Errorf("Error waiting on patch: 562 - %v (%v)",
			summaryBuffer.String(), err)
	}
	...
	for _, fileDetails := range fileStats {
		...
		if err != nil {
			if details[0] == "-" {
				...
				return nil, errors.Errorf("Error getting patch additions summary: %v", err)
			}
		}
		...
		if err != nil {
			if details[1] == "-" {
				...
				return nil, errors.Errorf("Error getting patch deletions summary: %v", err)
			}
		}
