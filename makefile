...
distArtifacts :=  ./public ./service/templates ./service/plugins
...
$(buildDir)/dist-race.tar.gz:$(buildDir)/make-tarball makefile $(raceBinaries) $(agentBinaries) $(clientBinaries)
...
race-%:$(buildDir)/output.%.race
	@grep -s -q -e "^(PASS|WARNING: DATA RACE)" $<
