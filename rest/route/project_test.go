...
func (s *ProjectGetSuite) TestPaginatorAttachesVarsFirst() {
	s.route.key = "projectA"
	s.route.limit = 1
	...
	s.Len(payload, 1)
	s.Equal("", (payload[0]).(*model.APIProject).Vars["x"])
	s.Equal("b", (payload[0]).(*model.APIProject).Vars["y"])
}

func (s *ProjectGetSuite) TestPaginatorAttachesVarsSecond() {
	s.route.key = "projectB"
	s.route.limit = 1
	...
	s.Len(payload, 1)
	s.Equal("a", (payload[0]).(*model.APIProject).Vars["x"])
	s.Equal("", (payload[0]).(*model.APIProject).Vars["y"])
}
